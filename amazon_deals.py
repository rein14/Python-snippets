...
my_parser.add_argument(
    ...
    metavar="searchterm",
    type=str,
    help="The item to be searched for. Use + for spaces",
)
...
def getdeals(soup):
    ...
    for item in products:
        ...
        short_title = item.find(
            "a", {"class": "a-link-normal a-text-normal"}
        ).text.strip()[:25]
        link = item.find("a", {"class": "a-link-normal a-text-normal"})["href"]
        try:
            saleprice = float(
                item.find_all("span", {"class": "a-offscreen"})[0]
                .text.replace("£", "")
                .replace(",", "")
                .strip()
            )
            oldprice = float(
                item.find_all("span", {"class": "a-offscreen"})[1]
                .text.replace("£", "")
                .replace(",", "")
                .strip()
            )
...
def getnextpage(soup):
    ...
    if not pages.find("li", {"class": "a-disabled a-last"}):
        url = "https://www.amazon.co.uk" + str(
            pages.find("li", {"class": "a-last"}).find("a")["href"]
        )
        return url
    else:
        return
